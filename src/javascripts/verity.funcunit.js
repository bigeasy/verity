/** A specialized driver for FuncUnit to work with Verity. */
(function($){
  ...
  FuncUnit.iframe = function (selector)  {
    FuncUnit._iframe = selector;
  }
  ...
  FuncUnit._open = function(url){
    ...
    if (newPage) {
      if (FuncUnit._iframe)  {
        var iframe = FuncUnit.jquery(FuncUnit._iframe).get(0);
        iframe.src = url;
        FuncUnit._window = iframe.contentWindow;
      }
      else {
